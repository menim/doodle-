...
var Lightbox = ((_temp = _class = function Lightbox(_ref) {
    ...
    if (!options.selector) {
        ...
        console.error(
            options.selector,
            "is not a string but a(n) " + _typeof(options.selector)
        );
    } else {
        ...
        this.links = Array.prototype.slice.call(
            document.querySelectorAll(this.id+" a" + options.selector)
        );
        this.offsets = [];
        this.nodes = {};
        this.imageIndex = null;
        if (this.links.length > 0) {
            this.createLightbox(id);
            this.createNodes(this.links, this.id);
            ...
            if (event) {
                if (event.target.className === "gallery__itemThumb") {
                    ...
                    items[i].style.transition =
                        "transform " + _this.slideSpeed + "ms ease-out";
                }
            }
        }
    };

    this.createNodes = function(links, id) {
        // Find all the lightbox nodes and add them to an object
        Object.assign(_this.nodes, {
            lightboxNode: document.querySelector(id + ".m-lightbox"),
            items: Array.prototype.slice.call(
                document.querySelectorAll(id + " .m-lightbox__slide")
            ),
            next: document.querySelector(id + " .m-lightbox__nextPrev--next"),
            prev: document.querySelector(id + " .m-lightbox__nextPrev--prev"),
            close: document.querySelector(id + " .m-lightbox__close")
        });

        Object.assign(_this.nodes, {
            counter: document.querySelector(id + " .m-lightbox__counter")
        });
    };
    ...
    this.renderImages = function(images) {
        var imagesLinks = images.map(function(item, index) {
            ...
            return (
                "\n                <li class='m-lightbox__slide' style='transform: translateX(" +
                offset +
                "vw)'>\n                    " +
                (_this.lazyload
                    ? "\n                        <img data-src='" +
